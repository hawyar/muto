...
(async function build() {
    // get this from miller's official release and pin to v6
    ...
    // we are using a wrapper https://github.com/hawyar/vitess-sqlparse, so we can get the build
    const sqlparserSemver = '0.1.4';
    const sqlparser = "sqlparser@" + "v" + sqlparserSemver;
    const sqlparserInBin = path.join(process.cwd(), 'node_modules', '.bin', sqlparser);
